...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if we have a valid task ID in the response
    if (!data.tasks || !data.tasks[0] || !data.tasks[0].id) {
      console.error("No task ID found in the response:", JSON.stringify(data, null, 2))
      return NextResponse.json({ error: "No task ID was returned from the API" }, { status: 500 })
    }
    
    // Extract the task ID and include it in the top level of our response
    const taskId = data.tasks[0].id
    const result = {
      id: taskId,
      status: data.tasks[0].status_code === 20100 ? "pending" : "failed",
      data: {
        target: cleanTarget,
        max_crawl_pages: options.max_crawl_pages || 100,
        max_crawl_depth: options.max_crawl_depth || 3
    ...
    console.log("Returning response with task ID:", taskId)
